...
from server import app, extract_genes, init_db
...
class TestServerAPI:
    ...
    def test_post_genome(self, client):
        ...
        response = client.post(
            "/api/genome",
            data=json.dumps({"genome": test_genome, "fitness": test_fitness, "client_id": "test_client"}),
    ...
    def test_post_genome_missing_data(self, client):
        ...
        response = client.post("/api/genome", data=json.dumps({}), content_type="application/json")
    ...
    def test_get_genome_after_submission(self, client):
        ...
        client.post(
            "/api/genome",
            data=json.dumps({"genome": test_genome, "fitness": test_fitness, "client_id": "test"}),
            content_type="application/json",
        )
    ...
    def test_pool_status_with_genomes(self, client):
        ...
        for genome, fitness in genomes:
            client.post(
                "/api/genome",
                data=json.dumps({"genome": genome, "fitness": fitness, "client_id": "test"}),
                content_type="application/json",
            )
    ...
    def test_genome_selection_bias(self, client):
        ...
        client.post(
            "/api/genome",
            data=json.dumps({"genome": [0.1] * 100, "fitness": 1.0, "client_id": "test"}),
            content_type="application/json",
        )

        client.post(
            "/api/genome",
            data=json.dumps({"genome": [0.9] * 100, "fitness": 100.0, "client_id": "test"}),
            content_type="application/json",
        )
...
class TestGeneExtraction:
    ...
    def test_extract_genes_multiple(self):
        ...
        genome = [0.96, 0.1, 0.2, 0.3, 0.04] + [0.5] * 10 + [0.96, 0.7, 0.8, 0.9, 0.6, 0.04]
        genes = extract_genes(genome)
